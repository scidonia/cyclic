...
(**
  ...
  Intuition:
  `ciu_jTy Σ Γ t u A` means: for any well-typed instantiation of the context Γ,
  `t` and `u` are indistinguishable by value-observation.
*)
...
Proof.
  split; intros Δ σ v _ Hv; exact Hv.
Qed.
...
Proof.
  ...
  split.
  - intros Δ σ v Hσ Hv. apply Huw with (Δ := Δ) (σ := σ); [exact Hσ|].
    apply Htu with (Δ := Δ) (σ := σ); [exact Hσ|exact Hv].
  - intros Δ σ v Hσ Hv. apply Hut with (Δ := Δ) (σ := σ); [exact Hσ|].
    apply Hwu with (Δ := Δ) (σ := σ); [exact Hσ|exact Hv].
Qed.
