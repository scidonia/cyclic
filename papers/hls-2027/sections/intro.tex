...
in Rocq; the LLM proposal step runs in Python and works correctly on
all tested examples, but the OCaml extraction bridge between them is
not yet wired (Section~\ref{sec:limits}).  The scrambling experiment
(Section~\ref{sec:eval}) uses 6 programs and 24 tests --- enough to
refute the hypothesis that the LLM relies solely on name recognition,
but not a statistical evaluation.  The type-index trace condition
extension (\S\ref{sec:design}) is specified and implemented but does
not yet enable any theorem that was previously out of reach.
...
\item \textbf{Mechanised $\omega$-rule} (Section~\ref{sec:limits}).
  ...
  reverse-append distribution.  The lemmas are hand-supplied in the Rocq tests; the LLM proposal
  step works in Python and succeeds on all tested examples; wiring
  through the OCaml extraction follows the same pattern as the existing
  generalisation oracle.
