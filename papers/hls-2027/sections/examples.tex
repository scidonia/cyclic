...
structure on both sides.  Commutativity requires a strengthened
induction hypothesis, discussed in Section~\ref{sec:reverse-append}.

\subsection{The $\omega$-rule: reverse-append fusion}
\label{sec:reverse-append}

The most intricate example demonstrates the $\omega$-rule.
The identity is:
\[
...
The standard SC terminates on both sides but produces
...
\item \textbf{Compiler soundness}: $\mathsf{exec}\;(\mathsf{compile}\;e)\;[] =
  [\mathsf{eval}\;e]$ with lemma $\mathsf{exec}\;(\mathsf{compile}\;e\;\mathit{code})\;\mathit{s}
  = \mathsf{exec}\;\mathit{code}\;(\cons\;(\mathsf{eval}\;e)\;\mathit{s})$.
\end{itemize}

\subsection{Commutativity of addition}

$\mathsf{plusL}\ m\ n = \mathsf{plusL}\ n\ m$ is a well-known hard case for
supercompilers.  Standard SC produces different residuals for the two
sides, each recursing on a different argument.  The LLM proposes two
unconditional lemmas (tested with original and scrambled names,
...
The sub-prover validates both by induction on $n$.  With these lemmas as
rewrite rules, the main proof produces identical residuals for both sides
(\texttt{plus\_commutativity} in \texttt{Commutativity.v},
\vmcompute{}).  Standard SC cannot prove it
(\texttt{std\_sc\_cannot\_prove\_comm}).

\subsection{Parity-filter interaction}

$\mathsf{any}\ \mathsf{evenp}\ (\mathsf{filter}\ \mathsf{oddp}\ l)
= \mathsf{false}$ requires discovering that $\mathsf{evenp}\ n =
\mathsf{false}$ whenever $\mathsf{oddp}\ n = \mathsf{true}$.  The sub-prover
discovers an unconditional version:

\[
\mathsf{any}\ \mathsf{evenp}\ (\cons\ n\ (\mathsf{filter}\ \mathsf{oddp}\ \mathit{xs}))
= \mathsf{any}\ \mathsf{evenp}\ (\mathsf{filter}\ \mathsf{oddp}\ \mathit{xs})
\]

This lemma is validated by the sub-prover (identical residuals), and the
main proof uses it to eliminate the $\mathsf{evenp}\ n$ check in the
filter's ``keep'' branch (\texttt{filter\_odd\_any\_even\_is\_false} in
\texttt{ParityLemma.v}).
...
\begin{tabular}{lcc}
\toprule
\textbf{File} & \textbf{Theorems} & \textbf{Mechanism} \\
\midrule
\texttt{HardExamples.v}          & 8  & AU only \\
\texttt{SortExamples.v}          & 9  & AU only \\
\texttt{MonadLaws.v}             & 6  & AU only \\
\texttt{IndexCalc.v}             & 10 & AU + $\omega$-rule \\
\texttt{OmegaExamples.v}         & 12 & $\omega$-rule \\
\texttt{LLMLemmaInAction.v}      & 4  & $\omega$-rule \\
\texttt{CompilerCorrectness.v}   & 5  & $\omega$-rule \\
\texttt{GradualCast.v}           & 5  & $\omega$-rule \\
\texttt{Commutativity.v}        & 3  & $\omega$-rule \\
\texttt{ParityLemma.v}          & 5  & $\omega$-rule \\
\texttt{DependentVec.v}          & 2  & dep. pruning \\
\texttt{DependentPruning.v}       & 2  & dep. pruning \\
\midrule
\textbf{Primary total}           & \textbf{71} & \\
\addlinespace
\textit{Infrastructure}          &     & \\
\texttt{CanonRoundtrip.v}        & 16 & lemma proof \\
\texttt{TypeIndexTrace.v}        & 4  & unit tests \\
\bottomrule
\end{tabular}
\end{center}
