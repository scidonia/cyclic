...
\paragraph{Supercompilation by evaluation.}
Recent work connects supercompilation to normalization-by-evaluation and partial evaluation \cite{jonesbook-1996-partial-evaluation}.
...
This discipline is standard in dependently typed proof assistants (Agda, Coq, Lean).
...
\subsection{Proof-carrying code and certified compilation}

\paragraph{Extracting programs from proofs.}
The Curry-Howard correspondence shows that proofs are programs and vice versa.
Proof assistants like Coq use extraction to produce executable programs from constructive proofs \cite{letouzey-2002-extraction}.
Our work takes a different perspective: we view supercompilation as producing a \emph{proof} (the cyclic derivation) from which the optimized program is extracted.

\paragraph{Translation validation vs proof transformation.}
Traditional certified compilers use translation validation: compile the program, then prove the output equivalent to the input \cite{pnueli-siegel-singerman-1998-translation-validation}.
Our approach is proof transformation: the supercompiler operates on proof graphs, and soundness follows from preservation of cyclic structure.
This is similar in spirit to recent work on intrinsically typed compilation \cite{pickard-hutton-gibbons-2021-calculating-compilers}.

\subsection{Summary}

This work makes several novel contributions:
\begin{enumerate}
\item \textbf{First supercompilation for dependent types}, addressing challenges of dependent contexts and type-directed transformation
\item \textbf{First focused sequent calculus for CoC + inductives}, cleanly separating deterministic driving from choice points
\item \textbf{Exact operational correspondence} between supercompilation and cyclic proof search, established via bisimulation and mechanized in Coq
\item \textbf{Post-hoc induction principle discovery}, showing how cyclic proofs enable flexible recursion schemes
\end{enumerate}
